...
var _ notifiers.NotifierRepository = (*smtpRepository)(nil)

type smtpRepository struct {
	db Database
}

// NewSmtpNotifierRepository instantiates a PostgreSQL implementation of notifier repository.
func NewSmtpNotifierRepository(db Database) notifiers.NotifierRepository {
	return &smtpRepository{
		db: db,
	}
}

func (sp smtpRepository) Save(ctx context.Context, nfs ...things.Notifier) ([]things.Notifier, error) {
	tx, err := sp.db.BeginTxx(ctx, nil)
	if err != nil {
		return []things.Notifier{}, errors.Wrap(errors.ErrCreateEntity, err)
	}

	q := `INSERT INTO smtp_notifiers (id, group_id, contacts, subtopics) VALUES (:id, :group_id, :contacts, :subtopics);`
...
func (sp smtpRepository) RetrieveByGroupID(ctx context.Context, groupID string) ([]things.Notifier, error) {
	if _, err := uuid.FromString(groupID); err != nil {
		return []things.Notifier{}, errors.Wrap(errors.ErrNotFound, err)
	}
	q := `SELECT id, group_id, contacts, subtopics FROM smtp_notifiers WHERE group_id = :group_id;`

	params := map[string]interface{}{
		"group_id": groupID,
	}

	rows, err := sp.db.NamedQueryContext(ctx, q, params)
	if err != nil {
...
func (sp smtpRepository) RetrieveByID(ctx context.Context, id string) (things.Notifier, error) {
	q := `SELECT group_id, contacts, subtopics FROM smtp_notifiers WHERE id = $1;`

	dbNf := dbNotifier{ID: id}
	if err := sp.db.QueryRowxContext(ctx, q, id).StructScan(&dbNf); err != nil {
		pgErr, ok := err.(*pgconn.PgError)
...
func (sp smtpRepository) Update(ctx context.Context, n things.Notifier) error {
	q := `UPDATE smtp_notifiers SET contacts = :contacts, subtopics = :subtopics WHERE id = :id;`
	...
	res, errdb := sp.db.NamedExecContext(ctx, q, dbNf)
	if errdb != nil {
...
func (sp smtpRepository) Remove(ctx context.Context, groupID string, ids ...string) error {
	for _, id := range ids {
		dbNf := dbNotifier{
			ID:      id,
			GroupID: groupID,
		}
		q := `DELETE FROM smtp_notifiers WHERE id = :id AND group_id = :group_id;`
		_, err := sp.db.NamedExecContext(ctx, q, dbNf)
		if err != nil {
