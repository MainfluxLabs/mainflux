...
func (n notifier) Send(to []string, _ protomfx.Notification) error {
	if len(to) < 1 {
...
func (nrm *notifierRepositoryMock) RetrieveByGroup(_ context.Context, groupID string, pm notifiers.PageMetadata) (res notifiers.NotifiersPage, err error) {
	nrm.mu.Lock()
	defer nrm.mu.Unlock()
