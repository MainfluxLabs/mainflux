...
func (ms *metricsMiddleware) AuthorizeThingKey(ctx context.Context, atr things.AuthorizeThingKeyReq) error {
	defer func(begin time.Time) {
		ms.counter.With("method", "authorize_thing_key").Add(1)
		ms.latency.With("method", "authorize_thing_key").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return ms.svc.AuthorizeThingKey(ctx, atr)
}
