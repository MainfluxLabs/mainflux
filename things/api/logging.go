...
func LoggingMiddleware(svc things.Service, logger log.Logger, auth domain.AuthClient) things.Service {
...
func (lm *loggingMiddleware) UpdateThingsMetadata(ctx context.Context, token string, things ...things.Thing) (err error) {
	defer func(begin time.Time) {
		...
		message := fmt.Sprintf("Method update_things_metadata by user %s, thing ids %s took %s to complete", email, ids, time.Since(begin))
		if err != nil {
			lm.logger.Warn(fmt.Sprintf("%s with error: %s.", message, err))
			return
		}
...
func (lm *loggingMiddleware) GetKeyByThingID(ctx context.Context, thingID string) (_ things.ThingKey, err error) {
	...
	return lm.svc.GetKeyByThingID(ctx, thingID)
}
