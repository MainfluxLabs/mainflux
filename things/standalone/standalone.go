...
var _ auth.Client = (*singleUserRepo)(nil)
...
func NewAuthService(email, token string) auth.Client {
...
func (repo singleUserRepo) Issue(ctx context.Context, id, email string, _ uint32) (string, error) {
	if repo.token != email {
...
func (repo singleUserRepo) Authorize(ctx context.Context, req domainauth.AuthzReq) error {
...
func (repo singleUserRepo) CreateDormantOrgInvite(ctx context.Context, token, orgID, inviteeRole string, groupInvites []domainauth.GroupInvite, platformInviteID string) error {
