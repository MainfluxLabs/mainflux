...
func (bs bootstrapService) Add(ctx context.Context, token string, cfg Config) (Config, error) {
	...
	id := cfg.ThingID
	thing, err := bs.thing(token, id)
	if err != nil {
		return Config{}, errors.Wrap(errAddBootstrap, err)
	}

	cfg.ThingID = thing.ID
	cfg.Owner = owner
	cfg.State = Inactive
	cfg.ThingKey = thing.Key
...
func (bs bootstrapService) UpdateConnections(ctx context.Context, token, id string, connections []string) error {
	...
	for _, d := range disconnect {
		disconnIDs := mfsdk.ConnectionIDs{
			ChannelID: d,
			ThingIDs:  []string{id},
		}

		if err := bs.sdk.Disconnect(disconnIDs, token); err != nil && !errors.Contains(err, mfsdk.ErrFailedDisconnect) {
...
func (bs bootstrapService) ChangeState(ctx context.Context, token, id string, state State) error {
	...
	case Active:
		for _, chID := range chIDs {
			...
			if err := bs.sdk.Connect(connIDs, token); err != nil {
				return errors.Wrap(ErrThings, err)
			}
		}
	case Inactive:
