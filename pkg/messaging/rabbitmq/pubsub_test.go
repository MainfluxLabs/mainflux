...
func TestPublisher(t *testing.T) {
	...
	topicChan := subscribe(t, ch, topic)
	subtopicChan := subscribe(t, ch, fmt.Sprintf("%s.%s", topic, subtopic))
...
func TestUnsubscribe(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			topic:     fmt.Sprintf("%s.%s", topic+"2", subtopic),
			clientID:  "clientid55",
			err:       nil,
			subscribe: true,
			handler:   handler{true, "clientid5"},
...
func TestPubSub(t *testing.T) {
	...
	for _, tc := range cases {
		subject := ""
		if tc.topic != "" {
			subject = fmt.Sprintf("%s.>", tc.topic)
		}
		...
		case nil:
			...
			sub := tc.topic
			if tc.subtopic != "" {
				sub = fmt.Sprintf("%s.%s", sub, tc.subtopic)
			}
			...
			err = pubsub.Unsubscribe(tc.clientID, subject)
			assert.Nil(t, err, fmt.Sprintf("%s got unexpected error: %s", tc.desc, err))
		default:
			assert.Equal(t, tc.err, err, fmt.Sprintf("%s: expected: %s, but got: %s", tc.desc, tc.err, err))
		}
