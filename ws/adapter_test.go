...
func TestPublish(t *testing.T) {
	...
	for _, tc := range cases {
		err := svc.Publish(context.Background(), apiutil.ThingKey{Type: apiutil.ThingKeyTypeInternal, Value: tc.thingKey}, tc.msg)
...
func TestSubscribe(t *testing.T) {
	...
	for _, tc := range cases {
		...
		err := svc.Subscribe(context.Background(), apiutil.ThingKey{Type: apiutil.ThingKeyPrefixInternal, Value: tc.thingKey}, tc.subtopic, c)
		assert.Equal(t, tc.err, err, fmt.Sprintf("%s: expected %s got %s\n", tc.desc, tc.err, err))
	}
}
...
func TestUnsubscribe(t *testing.T) {
	...
	for _, tc := range cases {
		pubsub.SetFail(tc.fail)
		err := svc.Unsubscribe(context.Background(), apiutil.ThingKey{Type: apiutil.ThingKeyTypeInternal, Value: tc.thingKey}, tc.subtopic)
		assert.Equal(t, tc.err, err, fmt.Sprintf("%s: expected %s got %s\n", tc.desc, tc.err, err))
	}
}
