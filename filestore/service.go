...
func (fs *filestoreService) SaveGroupFile(ctx context.Context, file io.Reader, token, groupID string, fi FileInfo) error {
	if _, err := fs.things.CanUserAccessGroup(ctx, &protomfx.UserAccessReq{Token: token, Id: groupID, Action: domain.GroupEditor}); err != nil {
		return err
	}
...
func (fs *filestoreService) ListGroupFiles(ctx context.Context, token, groupID string, fi FileInfo, pm PageMetadata) (FileGroupsPage, error) {
	if _, err := fs.things.CanUserAccessGroup(ctx, &protomfx.UserAccessReq{Token: token, Id: groupID, Action: domain.GroupViewer}); err != nil {
...
func (fs *filestoreService) identify(ctx context.Context, thingKey string) (string, error) {
	thingID, err := fs.things.Identify(ctx, &protomfx.ThingKey{Type: domain.KeyTypeInternal, Value: thingKey})
	if err != nil {
