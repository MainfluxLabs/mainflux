...
|--------|------------------------------|-----------------------------------------|
| GET    | `/senml`                     | List SenML messages for authenticated thing |
| GET    | `/senml/{publisherId}`       | List SenML messages for a specific publisher |
| DELETE | `/senml`                     | Delete SenML messages for authenticated thing |
| DELETE | `/senml/{publisherId}`       | Delete SenML messages for a specific publisher |
| POST   | `/senml/search`              | Search SenML messages with filters      |
| GET    | `/senml/export`              | Export SenML messages as JSON or CSV    |

### JSON Endpoints

| Method | Path                         | Description                             |
|--------|------------------------------|-----------------------------------------|
| GET    | `/json`                      | List JSON messages for authenticated thing |
| GET    | `/json/{publisherId}`        | List JSON messages for a specific publisher |
| DELETE | `/json`                      | Delete JSON messages for authenticated thing |
| DELETE | `/json/{publisherId}`        | Delete JSON messages for a specific publisher |
| POST   | `/json/search`               | Search JSON messages with filters       |
| GET    | `/json/export`               | Export JSON messages as JSON or CSV     |
...
### SenML-Specific Query Parameters

| Parameter    | Description                                                               |
|--------------|---------------------------------------------------------------------------|
| `name`       | Filter by SenML record name (`n` field)                                   |
| `v`          | Numeric value filter                                                      |
| `vb`         | Boolean value filter                                                      |
| `vs`         | String value filter                                                       |
| `vd`         | Data value filter                                                         |
| `comparator` | Comparison operator for `v`: `eq`, `lt`, `le`, `gt`, `ge`                |
| `aggregation`| Aggregation function: `min`, `max`, `avg`, `count`                        |
| `interval`   | Aggregation interval: `minute`, `hour`, `day`, `week`, `month`, `year`    |

### JSON-Specific Query Parameters

| Parameter  | Description                                          |
|------------|------------------------------------------------------|
| `filter`   | Free-form filter expression for JSON messages        |

### Export

Add `convert=csv` to any export request to receive results as a CSV file instead of JSON.
