...
func (ms *metricsMiddleware) UpdatePolicies(ctx context.Context, token, orgID string, gp ...auth.GroupsPolicy) error {
	defer func(begin time.Time) {
		ms.counter.With("method", "update_policies").Add(1)
		ms.latency.With("method", "update_policies").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return ms.svc.UpdatePolicies(ctx, token, orgID, gp...)
}
