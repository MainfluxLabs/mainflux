...
func (ms *metricsMiddleware) Refresh(ctx context.Context, refreshToken string) (auth.Key, string, error) {
	defer func(begin time.Time) {
...
func (ms *metricsMiddleware) ListAPIKeys(ctx context.Context, token string, pm auth.PageMetadata) (auth.KeysPage, error) {
	defer func(begin time.Time) {
...
func (ms *metricsMiddleware) ListOrgMemberships(ctx context.Context, token, orgID string, pm auth.PageMetadata) (auth.OrgMembershipsPage, error) {
	defer func(begin time.Time) {
...
func (ms *metricsMiddleware) ListOrgInvitesByUser(ctx context.Context, token, userType, userID string, pm auth.PageMetadata) (auth.OrgInvitesPage, error) {
	defer func(begin time.Time) {
...
func (ms *metricsMiddleware) ListOrgInvitesByOrg(ctx context.Context, token, orgID string, pm auth.PageMetadata) (auth.OrgInvitesPage, error) {
	defer func(begin time.Time) {
...
func (ms *metricsMiddleware) GetDormantOrgInviteByPlatformInvite(ctx context.Context, platformInviteID string) (auth.OrgInvite, error) {
	defer func(begin time.Time) {
		ms.counter.With("method", "get_dormant_org_invite_by_platform_invite").Add(1)
		ms.latency.With("method", "get_dormant_org_invite_by_platform_invite").Observe(time.Since(begin).Seconds())
	}(time.Now())
	return ms.svc.GetDormantOrgInviteByPlatformInvite(ctx, platformInviteID)
}
