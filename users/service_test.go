...
func TestInvitePlatformMember(t *testing.T) {
	...
	existingInvite, err := svc.InvitePlatformMember(context.Background(), tokenAdmin, inviteRedirectPath, "existingUser@example.com")
	...
	for desc, tc := range cases {
		_, err := svc.InvitePlatformMember(context.Background(), tc.token, inviteRedirectPath, tc.email)
		assert.True(t, errors.Contains(err, tc.err), fmt.Sprintf("%s: expected %s got %s\n", desc, tc.err, err))
	}
}
...
func TestRevokePlatformInvite(t *testing.T) {
	...
	pendingInvite, err := svc.InvitePlatformMember(context.Background(), tokenAdmin, inviteRedirectPath, "test1@example.com")
	assert.Nil(t, err, fmt.Sprintf("Creating platform invite expected to succeed: %s", err))

	acceptedInvite, err := svc.InvitePlatformMember(context.Background(), tokenAdmin, inviteRedirectPath, "test2@example.com")
	assert.Nil(t, err, fmt.Sprintf("Creating platform invite expected to succeed: %s", err))
...
func TestListPlatformInvites(t *testing.T) {
	...
	for i := uint64(0); i < n; i++ {
		_, err := svc.InvitePlatformMember(context.Background(), tokenAdmin, inviteRedirectPath, fmt.Sprintf("test%d@example.com", i))
		assert.Nil(t, err, fmt.Sprintf("Creating platform invite expected to succeed: %s", err))
	}
...
func TestValidatePlatformInvite(t *testing.T) {
	...
	pendingInvite2, err := svc.InvitePlatformMember(context.Background(), tokenAdmin, inviteRedirectPath, "test11@example.com")
	assert.Nil(t, err, fmt.Sprintf("Creating platform invite expected to succeed: %s", err))

	revokedInvite, err := svc.InvitePlatformMember(context.Background(), tokenAdmin, inviteRedirectPath, "test2@example.com")
	assert.Nil(t, err, fmt.Sprintf("Creating platform invite expected to succeed: %s", err))
	...
	acceptedInvite, err := svc.InvitePlatformMember(context.Background(), tokenAdmin, inviteRedirectPath, "test3@example.com")
	assert.Nil(t, err, fmt.Sprintf("Creating platform invite expected to succeed: %s", err))
	err = svc.ValidatePlatformInvite(context.Background(), acceptedInvite.ID, acceptedInvite.InviteeEmail)
	assert.Nil(t, err, fmt.Sprintf("Validating platform invite expected to succeed: %s", err))
