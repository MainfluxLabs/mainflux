...
func (ms *metricsMiddleware) RegisterByInvite(ctx context.Context, user users.User, inviteID, orgInviteRedirectPath string) (string, error) {
	defer func(begin time.Time) {
	...
	return ms.svc.RegisterByInvite(ctx, user, inviteID, orgInviteRedirectPath)
}
...
func (ms *metricsMiddleware) ListUsersByIDs(ctx context.Context, ids []string, pm users.PageMetadata) (users.UserPage, error) {
	...
	return ms.svc.ListUsersByIDs(ctx, ids, pm)
}
...
func (ms *metricsMiddleware) ChangePassword(ctx context.Context, token, email, password, oldPassword string) error {
	...
	return ms.svc.ChangePassword(ctx, token, email, password, oldPassword)
}
...
func (ms *metricsMiddleware) ViewPlatformInvitePublic(ctx context.Context, inviteID string) (users.PlatformInvite, error) {
	defer func(begin time.Time) {
		ms.counter.With("method", "view_platform_invite_public").Add(1)
		ms.latency.With("method", "view_platform_invite_public").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return ms.svc.ViewPlatformInvitePublic(ctx, inviteID)
}
