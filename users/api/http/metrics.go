...
func (ms *metricsMiddleware) SelfRegister(ctx context.Context, user users.User, path string) (string, error) {
	defer func(begin time.Time) {
		ms.counter.With("method", "self_register").Add(1)
		ms.latency.With("method", "self_register").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return ms.svc.SelfRegister(ctx, user, path)
}
...
func (ms *metricsMiddleware) GenerateResetToken(ctx context.Context, email, path string) error {
	defer func(begin time.Time) {
	...
	return ms.svc.GenerateResetToken(ctx, email, path)
}
...
func (ms *metricsMiddleware) SendPasswordReset(ctx context.Context, path, email, token string) error {
	defer func(begin time.Time) {
	...
	return ms.svc.SendPasswordReset(ctx, path, email, token)
}
