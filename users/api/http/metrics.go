...
func (ms *metricsMiddleware) ListUsersByIDs(ctx context.Context, ids []string, order string, dir string) (users.UserPage, error) {
	defer func(begin time.Time) {
		ms.counter.With("method", "list_users_by_ids").Add(1)
		ms.latency.With("method", "list_users_by_ids").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return ms.svc.ListUsersByIDs(ctx, ids, order, dir)
}
