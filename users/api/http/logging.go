...
func (lm *loggingMiddleware) CreatePlatformInvite(ctx context.Context, token, redirectPath, email, orgID, role string, gis []auth.GroupInvite) (invite users.PlatformInvite, err error) {
	defer func(begin time.Time) {
	...
	return lm.svc.CreatePlatformInvite(ctx, token, redirectPath, email, orgID, role, gis)
}
