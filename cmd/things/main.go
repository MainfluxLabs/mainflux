...
func newService(ac protomfx.AuthServiceClient, uc protomfx.UsersServiceClient, dbTracer opentracing.Tracer,
	...
	invitesRepo := postgres.NewGroupInviteRepository(db)
	invitesRepo = tracing.InvitesRepositoryMiddleware(dbTracer, invitesRepo)
	...
	svc := things.New(ac, uc, thingsRepo, profilesRepo, groupsRepo, invitesRepo, groupMembershipsRepo, profileCache, thingCache, groupCache, idProvider, thingsEmailer, cfg.inviteDuration)
