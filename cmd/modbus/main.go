...
func main() {
	...
	svc := newService(things, pub, dbTracer, db, logger, auth)
...
func newService(ts domain.ThingsClient, pub messaging.Publisher, dbTracer opentracing.Tracer, db *sqlx.DB, logger logger.Logger, ac domain.AuthClient) modbus.Service {
