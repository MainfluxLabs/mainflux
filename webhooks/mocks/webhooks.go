...
func (wrm *webhookRepositoryMock) RetrieveByGroup(_ context.Context, groupID string, pm webhooks.PageMetadata) (webhooks.WebhooksPage, error) {
	wrm.mu.Lock()
	defer wrm.mu.Unlock()
...
func (wrm *webhookRepositoryMock) RetrieveByThing(_ context.Context, thingID string, pm webhooks.PageMetadata) (webhooks.WebhooksPage, error) {
	wrm.mu.Lock()
	defer wrm.mu.Unlock()
	var items []webhooks.Webhook
...
func (wrm *webhookRepositoryMock) RemoveByThing(_ context.Context, thingID string) error {
	wrm.mu.Lock()
	defer wrm.mu.Unlock()

	for id, wh := range wrm.webhooks {
		if wh.ThingID == thingID {
			delete(wrm.webhooks, id)
		}
	}
...
func (wrm *webhookRepositoryMock) RemoveByGroup(_ context.Context, groupID string) error {
	wrm.mu.Lock()
	defer wrm.mu.Unlock()
