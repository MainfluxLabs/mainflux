...
func toInvocationLimitedLuaFunc(luaFunc lua.Function, maxInvocations uint) lua.Function {
	...
	limitedFunc := func(ls *lua.State) int {
		if currentInvocations >= maxInvocations {
			// TODO: return (false, <error_message>) to indicate to caller that they've exhausted all invocations??
			return 0
		}
...
// For each passed Lua script, create a new Lua envrionment and execute the associated script which processes the msg Mainflux message. msg.Payload is ignored.
// parsedPayload represents the entire parsed payload of the associated Message.
...
func (rs *rulesService) processLuaScripts(ctx context.Context, msg *protomfx.Message, parsedPayload any, scripts ...LuaScript) {
	...
	for _, script := range scripts {
		for _, subPayload := range payloads {
			...
			run, err := env.execute()
			if err != nil {
				rs.logger.Info(fmt.Sprintf("executing rule script with id %s failed with error %v", env.script.ID, err))
			}
