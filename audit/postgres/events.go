...
func (r *eventRepository) SaveEvent(ctx context.Context, e audit.Event) error {
	q := `INSERT INTO events (id, occurred_at, operation, actor_user_id, actor_user_email, data)
	      VALUES (:id, :occurred_at, :operation, :actor_user_id, :actor_user_email, :data)`
...
func (r *eventRepository) RetrieveEvents(ctx context.Context, pm audit.PageMetadata) (audit.EventsPage, error) {
	...
	where := dbutil.BuildWhereClause(emailQ, opQ, dataQ)
	order := dbutil.GetOrderQuery(pm.Order)
	dir := dbutil.GetDirQuery(pm.Dir)
	olq := dbutil.GetOffsetLimitQuery(pm.Limit)

	query := fmt.Sprintf(
		`SELECT id, occurred_at, operation, actor_user_id, actor_user_email, data FROM events %s ORDER BY %s %s %s`,
		where, order, dir, olq,
	)
